...
[[csl::vertex]] VertexOutput vertex_main()
{
      ...
      const vector<f32, 4> position = scene.ubo->view * scene.ubo->model * vector<f32, 4>{ input.position.x, input.position.y, input.position.z, 1.0f };
      output.normal                 = mat3(scene.ubo->model) * input.normal;

      const vector<f32, 3> light_position = vector<f32, 3>{ 1.0f, -1.0f, 1.0f };
