...
[[csl::fragment]] FragmentOutput fragment_main()
{
      ...
      reflected.yz = reflected.yz * -1.0f;

      const vector<f32, 4> coordinate = vector<f32, 4>{ reflected.x, reflected.y, reflected.z, static_cast<f32>(resource.ubo->cube_map_index) };
      const vector<f32, 4> color      = resource.cube_map_array.sample_lod(coordinate, resource.ubo->lod_bias);
      ...
      const vector<f32, 3> ambient  = vector<f32, 3>{ 0.5f, 0.5f, 0.5f } * color.rgb;
      ...
      const vector<f32, 3> lit = ambient + diffuse * color.rgb + specular;
