...
[[csl::fragment]] FragmentOutput fragment_main()
{
      ...
      const vector<f32, 4> sampled = textures.color.sample_bias(input.uv, input.lod_bias);
      ...
      const f32            specular = pow(max(dot(reflected, view), 0.0f), 16.0f) * sampled.w;

      const vector<f32, 3> lit = diffuse * sampled.xyz + specular;
