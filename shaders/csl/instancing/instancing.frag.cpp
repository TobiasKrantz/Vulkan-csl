...
[[csl::fragment]] FragmentOutput fragment_main()
{
      ...
      const vector<f32, 4> color     = textures.sampler_array.sample(input.uv) * vector<f32, 4>{ input.color.x, input.color.y, input.color.z, 1.0f };
      ...
      const vector<f32, 3> diffuse  = max(dot(normal, light), 0.1f) * input.color;
      const vector<f32, 3> specular = dot(normal, light) > 0.0f ? pow(max(dot(reflected, view), 0.0f), 16.0f) * vector<f32, 3>{ 0.75f, 0.75f, 0.75f } * color.x : vector<f32, 3>{ 0.0f, 0.0f, 0.0f };
