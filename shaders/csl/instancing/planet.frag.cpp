...
[[csl::fragment]] FragmentOutput fragment_main()
{
      ...
      const vector<f32, 4> color     = textures.color_map.sample(input.uv) * vector<f32, 4>{ input.color.x, input.color.y, input.color.z, 1.0f } * 1.5f;
      ...
      const vector<f32, 3> specular = pow(max(dot(reflected, view), 0.0f), 4.0f) * vector<f32, 3>{ 0.5f, 0.5f, 0.5f } * color.x;
