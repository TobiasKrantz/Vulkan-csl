...
[[csl::closest_hit]] void closest_hit_main()
{
      ...
      vector<f32, 3> color = resources_.textures[nonuniform(u32(geometry_node.texture_index_base_color))].sample_lod(triangle.uv, 0.0f).xyz;
      if (geometry_node.texture_index_occlusion > -1)
      {
            const f32 occlusion = resources_.textures[nonuniform(u32(geometry_node.texture_index_occlusion))].sample_lod(triangle.uv, 0.0f).x;
            color *= occlusion;
      }
