...
struct Scene
{
      ...
      [[csl::uniform_buffer(1, 0)]] const UniformInline* material;
};
...
static vector<f32, 3> brdf(vector<f32, 3> l, vector<f32, 3> v, vector<f32, 3> n, f32 metallic, f32 roughness, vector<f32, 3> material_color)
{
      ...
      if (dot_nl > 0.0f)
      {
            ...
            const f32            d          = d_ggx(dot_nh, rroughness);
...
[[csl::fragment]] FragmentOutput fragment_main()
{
      ...
      const vector<f32, 3> material_color = vector<f32, 3>{ scene.material->r, scene.material->g, scene.material->b };

      const vector<f32, 3> n         = normalize(input.normal);
      const vector<f32, 3> v         = normalize(scene.ubo->camera_position - input.world_position);
      const f32            roughness = scene.material->roughness;

      const vector<f32, 3> light_position = vector<f32, 3>{ 0.0f, 0.0f, 10.0f };
      const vector<f32, 3> l              = normalize(light_position - input.world_position);
      const vector<f32, 3> lo             = brdf(l, v, n, scene.material->metallic, roughness, material_color);

      vector<f32, 3> color = material_color * scene.material->ambient;
      color                = color + lo;
