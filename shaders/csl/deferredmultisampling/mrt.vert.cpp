...
[[csl::vertex]] VertexOutput vertex_main()
{
      ...
      const vector<f32, 4> base           = vector<f32, 4>{ input.position.x, input.position.y, input.position.z, 1.0f };
      const vector<f32, 4> local_position = base + scene.ubo->instance_position[instance_index()];
      ...
      output.tangent                        = normal_matrix * normalize(input.tangent);
